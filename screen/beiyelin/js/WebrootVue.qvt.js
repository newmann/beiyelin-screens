...
//                    console.log('>>>> loadId',self.loadId)
//                    console.log('>>>> linkHref',self.linkHref)
//                    if (self.loadId && self.loadId.length > 0) {
//                        self.$root.loadContainer(self.loadId, self.href);
//                    } else {
//                        if (event.ctrlKey || event.metaKey) {
//                            window.open(self.linkHref, "_blank");
//                        } else {
//                            self.$root.setUrl(self.linkHref);
//                        }
//                    }
//                  }).onOk(() => {
...
Vue.component('m-container-dialog', {
    ...
    '<span>' +
        ...
        '</template> '+
        '<template v-else> ' +
            '<q-drawer v-model="isShown" :id="id" :title="title" :color="color" :width="getWidth()" overlay elevated side="right" > '+
            '<q-scroll-area class="fit"> '+
            '<q-card flat > ' +
                '<q-card-actions>' +
                    '<h5 class="q-pl-sm non-selectable">{{title}}</h5><q-space></q-space>' +
                    '<q-btn icon="close" flat round dense @click="hide()"></q-btn>' +
                '</q-card-actions>'+
...
/*
TODO:用quasar的q-popup-edit来替代
    1、需要编写template，根据不同的widgetType来决定采用什么控件
    2、提交到后台，通过ajax来提交
*/
...
//                targetEl.parents(".form-group").find(".select2-selection").addClass("is-changed");
                targetEl.parents(".q-field__control-container").find(".q-field__label").addClass("value-changed");
...
//                targetEl.parents(".form-group").find(".select2-selection").removeClass("is-changed");
                targetEl.parents(".q-field__control-container").find(".q-field__label").removeClass("value-changed");
...
*/
    ...
    mounted: function() {
        ...
        /* TODO: should not need to watch input fields any more,在每个component中进行处理了
        // watch changed fields
        jqEl.find(':input').on('change', this.fieldChange);
        jqEl.children('select').on('change', this.fieldChange);
...
Vue.component('m-date-time', {
    ...
    '<q-input dense  stack-label label-slot v-bind:value="value" v-on:input="$emit(\'input\', $event)" :rules="rules"' +
        ...
        '<template v-slot:prepend v-if="type==\'date\' || type==\'date-time\' || !type">' +
            '<q-icon name="event" class="cursor-pointer">' +
                '<q-popup-proxy ref="qDateProxy" transition-show="scale" transition-hide="scale">' +
                    '<q-date v-bind:value="value" v-on:input="$emit(\'input\', $event)" :mask="formatVal" @input="dateInputFn"></q-date>' +
                '</q-popup-proxy>' +
            '</q-icon>' +
        '</template>' +
        '<template v-slot:append v-if="type==\'time\' || type==\'date-time\' || !type">' +
            '<q-icon name="access_time" class="cursor-pointer">' +
                '<q-popup-proxy ref="qTimeProxy" transition-show="scale" transition-hide="scale">' +
                    '<q-time v-bind:value="value" v-on:input="$emit(\'input\', $event)" :mask="formatVal" format24h @input="timeInputFn"></q-time>' +
                '</q-popup-proxy>' +
            '</q-icon>' +
        '</template>' +
        '<template v-slot:after><slot name="after"></slot></template>' +
        '<template v-slot:label> '+
            '<div  :class="[isChanged?\'value-changed\':\'\']"> {{label}} '+
    ...
    data: function() { return { isChanged: false, initValue: this.value } },
    methods: {
        ...
        dateInputFn:function(value){
            console.log(value);
            this.$refs.qDateProxy.hide()
            this.isChanged= ((value===null)? '' : value.toString()) != ((this.initValue === null) ? '' : this.initValue.toString());
        },
        timeInputFn:function(value){
            console.log(value);
...
Vue.component('m-date-period', {
    ...
    '<div v-else class="row"><q-input dense  stack-label :label="label" v-model="fields[name+\'_pdate\']"' +
        ...
        '<template v-slot:before>' +
            '<q-select class="q-pr-xs" dense  options-dense emit-value map-options v-model="fields[name+\'_poffset\']" :name="name+\'_poffset\'"' +
                ' stack-label label="Offset" :options="dateOffsets" :form="form" behavior="menu"></q-select>' +
            '<q-select dense  options-dense emit-value map-options v-model="fields[name+\'_period\']" :name="name+\'_period\'"' +
...
Vue.component('m-display', {
    ...
    props: { value:String, display:String, valueUrl:String, valueParameters:Object, dependsOn:Object, dependsOptional:Boolean, valueLoadInit:Boolean,
        fields:{type:Object}, tooltip:String, label:String, labelWrapper:Boolean, name:String, id:String ,displayType:String},
    data: function() { return { curDisplay:this.display, loading:false } },
    template:
        '<q-input v-if="labelWrapper" dense  readonly stack-label autogrow :value="displayValue" :label="label" :id="id" :name="name" :loading="loading">' +
            '<q-tooltip v-if="tooltip">{{tooltip}}</q-tooltip>' +
        '</q-input>' +
        '<span v-else :id="id">' +
            '<q-tooltip v-if="tooltip">{{tooltip}}</q-tooltip><slot></slot>' +
            '<span v-if="displayType == \'chip\'" >' +
    ...
    mounted: function() {
        if (this.valueUrl && this.valueUrl.length) {
            ...
            for (var doParm in dependsOnMap) { if (dependsOnMap.hasOwnProperty(doParm)) {
                if (this.fields) {
//update by hxs 20230822
//                    this.$watch('fields.' + doParm, function() { this.populateFromUrl({term:this.value}); });
                    this.$watch('fields.' +  dependsOnMap[doParm], function() { this.populateFromUrl({term:this.value}); });
                } else {
...
Vue.component('m-option-group', {
    name: 'mOptionGroup',
    template:'<q-field dense  stack-label label-slot :disable="disable" :form="form"> '+
        '<q-tooltip v-if="tooltip">{{tooltip}}</q-tooltip>' +
        '<template v-slot:control>' +
        '<q-option-group dense inline size="xs" :name="name" :id="id" v-bind:value="value" v-on:input="$emit(\'input\', $event)" :options="options" '+
            ':type="type" @input="inputFn"> ' +
        '</q-option-group>' +
        '</template>'+
        '<template v-slot:label> '+
            '<div :class="[isChanged?\'value-changed\':\'\']"> {{label}} '+
    ...
    props: { value:[Array,String], options:{type:Array,'default':function(){return [];}},
        form:String, fields:{type:Object}, type:{type:String,'default':'radio'},
        tooltip:String, label:String, name:String, id:String, disable:Boolean, required:Boolean
     },
    data: function() { return { isChanged: false, initValue:[Array,String] } },
    methods: {
...
Vue.component('m-drop-down', {
    ...
    data: function() { return { curOptions:this.options, allOptions:this.options, lastVal:null, lastSearch:null, loading:false, isChanged: false, initValue:[Array,String] } },
    computed: {
    ...
    template:
        ...
        '<q-select ref="qSelect" v-bind:value="value" v-on:input="handleInput($event)"' +
                ...
                ' stack-label label-slot :loading="loading" :options="curOptions" @input="inputFn" :moquiKey="upperCaseMoquiKey"  >' +
            '<q-tooltip v-if="tooltip">{{tooltip}}</q-tooltip>' +
...
//            console.log(value);
//            this.isChanged = this.initValue.includes(value); //删除老的项目就设为修改
            ...
            if (curValueArr.length !== newValueArr.length){
                //这里发出input消息，自己的过程inputFn接收不到，所以直接修改
                this.isChanged= ((newValueArr===null)? '' : newValueArr.toString()) != ((this.initValue === null) ? '' : this.initValue.toString());
        ...
        clearAll: function() {
            ...
            this.isChanged = (this.initValue != null);
...
Vue.component('m-text-line', {
    ...
    props: { value:String, type:{type:String,'default':'text'}, id:String, name:String, size:String, fields:{type:Object},
        ...
        defaultUrl:String, defaultParameters:Object, dependsOn:Object, dependsOptional:Boolean, defaultLoadInit:Boolean, required:Boolean,
        autogrow:Boolean, rows:String, cols:String,moquiKey:String },
    data: function() { return { loading:false, isChanged:false, initValue:String } },
    template:
        '<q-input dense stack-label label-slot :prefix="prefix" v-bind:value="value" v-on:input="$emit(\'input\', $event)" :type="type"' +
                ' :id="id" :name="name" :size="size" :loading="loading" :rules="rules" :disable="disable" ' +
                ' :mask="mask" :fill-mask="fillMask" :reverse-fill-mask="reverseFillMask" :rows="rows" :cols="cols" :autogrow="autogrow" ' +
                ' autocapitalize="off" autocomplete="off" @input="inputFn" :moquiKey="upperCaseMoquiKey" >' +
                '<template v-slot:label> '+
                '<div :class="[isChanged?\'value-changed\':\'\']"> {{label}} '+
                '<span v-if ="moquiKey"> ( <span style="text-decoration:underline">{{upperCaseMoquiKey}}</span> )</span> '+
                '<em class="text-red" v-if ="required"> * </em>' +
...
moqui.webrootVue = new Vue({
    ...
    methods: {
        ...
        */
        ...
        // keyboard process
        addProcessKeyDownListener() {
            if (window.__moquiKeyProcessBind__) return;
            window.addEventListener("keydown", this.processKeyDown);
            window.__moquiKeyProcessBind__ = true;
        },
        removeProcessKeyDownListener() {
          window.removeEventListener("keydown", this.processKeyDown);
          window.__moquiKeyProcessBind__ = false;
        },
               ...
               //alt key
                if(altDown){
                    if((keyCode>=65 && keyCode <=90) ||(keyCode>=97 && keyCode <=122)){
                        ...
                        console.log($(`input[moquiKey="${keyStr}"]`));
                        $(`input[moquiKey="${keyStr}"]`).focus();
                ...
                //Enter key,在输入框进行处理，还没有测试过radio、checkbox等
                if(keyCode === 13){
//                    console.log(e);
                    ...
                    let inputList = e.target.form.getElementsByTagName("input");
                    ...
                    for(let i =0; i< inputList.length; i++){
                        if(inputList[i].name === curInput.name){
                            curIndex = i;
                            break;
                        }
                    }
                    if (curIndex == inputList.length - 1){
                        curIndex = -1;
                    }
    ...
    mounted: function() {
        ...
        //keyboard process
        this.addProcessKeyDownListener();
    },
    beforeDestroy: function() {
        this.sessionTokenBc.close();

        //keyboard process
        this.removeProcessKeyDownListener();
    }
...
//Keystroke
keystrokes.bindKey('alt>b', () => console.log('you pressed alt+b'))
